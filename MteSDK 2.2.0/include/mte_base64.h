...
MTE_SHARED
size_t mte_base64_encode_bytes(size_t in_bytes);
...
MTE_SHARED
size_t mte_base64_encode(const void *data, size_t bytes, char *encoded);

/* Returns the required decode buffer length for the given input Base64
   length. */
MTE_SHARED
size_t mte_base64_decode_bytes(size_t in_bytes);
...
MTE_SHARED
size_t mte_base64_decode(const void *base64, size_t bytes, void *decoded);
